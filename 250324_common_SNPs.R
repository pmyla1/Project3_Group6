...
setwd('/Users/lukearcher/Desktop/230324_lyrata_arenosa/')

##define input files
arenosa_lyrata_AFs<-read_tsv('AFs_only_lyrata_arenosa.tsv')
head(arenosa_lyrata_AFs)
...
head(cleaned_arenosa_lyrata_AFs)
##################################

####################################
##SUBSETTING THE DATA INTO SMALLER DATAFRAMES BASED ON SCAFFOLD
...
##do the same for all scaffolds, making sure you check each one
...
##################################
##PLOT THE AF VS POSITION ALONG THE CHROMOSOME
threshold_arenosa1<-quantile(chrom1$AF_arenosa,prob=0.75)+1.5*IQR(chrom1$AF_arenosa)
threshold_arenosa1

ggplot(chrom1,aes(POS,AF_arenosa,colour=threshold_arenosa1))+
  geom_point(alpha=0.5)+
  theme_bw()+
  labs(title='Allele frequency vs position\non chromosome 1: A. arenosa',
       x='Position along chromosome 1',y='Allele Frequency')+
  theme(title=element_text(face='bold',size=11),
        panel.grid.minor.x=element_blank(),
...
###different plot
arenosa_density<-ggplot(arenosa_lyrata_MAF2.5,aes(x=AF_arenosa))+
  geom_density(fill='orange',alpha=0.8)+
  geom_vline(xintercept=mean(arenosa_lyrata_MAF2.5$AF_arenosa),
             linetype=2,colour=2)+
  theme_bw()+
  labs(title="Density of genome wide SNPs in A. arenosa",
       x="A. arenosa AF",y="Density")+
  theme(title=element_text(face='bold',size=11),
        panel.grid.minor=element_blank())

lyrata_density<-ggplot(arenosa_lyrata_MAF2.5,aes(x=AF_lyrata))+
  geom_density(fill='violet',alpha=0.8)+
  geom_vline(xintercept=mean(arenosa_lyrata_MAF2.5$AF_lyrata),
             linetype=2,colour=2)+
  theme_bw()+
  labs(title="Density of genome wide SNPs in A. lyrata",
       x="A. lyrata AF",y="Density")+
  theme(title=element_text(face='bold',size=11),
        panel.grid.minor=element_blank())
##multi-panel plot
grid.arrange(arenosa_density,lyrata_density,nrow=2)

##############
##plot the A. arenosa AF vs A. lyrata AF 
ggplot(arenosa_lyrata_MAF2.5,aes(AF_arenosa,AF_lyrata))+
  geom_point(alpha=0.05,position='jitter',colour=2)+
  theme_bw()+
  labs(title="Genome wide SNPs between\nA. arenosa and A. lyrata",
       y="A. lyrata AF",x="A. arenosa AF")+
  theme(title=element_text(face='bold',size=11),
        panel.grid.minor=element_blank())
...
head(arenosa_lyrata_MAF2.5)
####make thresholds for all scaffolds
...
##############
##PLOTTING OF AD DIFFERENCES BETWEEN SPECIES AND SCAFFOLDS
...
########PLOT AF DIFFERENCE VS POSITION ON ALL CHROMOSOMES
...
##SUBSETTING THE DATA INTO SMALLER DATAFRAMES BASED ON SCAFFOLD
##subset the data per chromosome/scaffold 
chrom1<-subset(arenosa_lyrata_MAF2.5,CHROM=='scaffold_1')
head(chrom1)
#compute mean AFs per site
arenosa_mean1<-mean(chrom1$AF_arenosa)
arenosa_mean1
...
lyrata_mean1
##do the same for all scaffolds, making sure you check each one
chrom2<-subset(arenosa_lyrata_MAF2.5,CHROM=='scaffold_2')
head(chrom2)

arenosa_mean2<-mean(chrom2$AF_arenosa)
lyrata_mean2<-mean(chrom2$AF_lyrata)

chrom3<-subset(arenosa_lyrata_MAF2.5,CHROM=='scaffold_3')
head(chrom3)

arenosa_mean3<-mean(chrom3$AF_arenosa)
lyrata_mean3<-mean(chrom3$AF_lyrata)

chrom4<-subset(arenosa_lyrata_MAF2.5,CHROM=='scaffold_4')
head(chrom4)

arenosa_mean4<-mean(chrom4$AF_arenosa)
lyrata_mean4<-mean(chrom4$AF_lyrata)

chrom5<-subset(arenosa_lyrata_MAF2.5,CHROM=='scaffold_5')
head(chrom5)

arenosa_mean5<-mean(chrom5$AF_arenosa)
lyrata_mean5<-mean(chrom5$AF_lyrata)

chrom6<-subset(arenosa_lyrata_MAF2.5,CHROM=='scaffold_6')
head(chrom6)

arenosa_mean6<-mean(chrom6$AF_arenosa)
lyrata_mean6<-mean(chrom6$AF_lyrata)

chrom7<-subset(arenosa_lyrata_MAF2.5,CHROM=='scaffold_7')
head(chrom7)

arenosa_mean7<-mean(chrom7$AF_arenosa)
lyrata_mean7<-mean(chrom7$AF_lyrata)

chrom8<-subset(arenosa_lyrata_MAF2.5,CHROM=='scaffold_8')
head(chrom8)
...
head(top_1PCT_AF_outliers_chrom8)
###################################
...
#####################
##MERGE THE 8 TOP 1% SCAFFOLDS INTO ONE DF 
###make a top 1% outliers data frame from the 8 individual scaffolds
top_1PCT_whole_genome<-do.call('rbind',list(top_1PCT_AF_outliers_chrom1,top_1PCT_AF_outliers_chrom2,
                                     top_1PCT_AF_outliers_chrom3,top_1PCT_AF_outliers_chrom4,
                                     top_1PCT_AF_outliers_chrom5,top_1PCT_AF_outliers_chrom6,
                                     top_1PCT_AF_outliers_chrom7,top_1PCT_AF_outliers_chrom8))
top_1PCT_whole_genome<-as_tibble(top_1PCT_whole_genome)
head(top_1PCT_whole_genome)
dim(top_1PCT_whole_genome)
#########################

#############################
##MAKING A BED FILE FROM THE DATA USING 100BP SNP WINDOWS
top_1PCT_WG_BED<-top_1PCT_whole_genome%>%select(c("CHROM","POS"))
head(top_1PCT_WG_BED)

##make a function for the start of the SNP bin
BIN_START<-function(x){
  return(round(x,digits=-2)-100)
}
##make a function for the end of the SNP bin
BIN_END<-function(x){
  return(round(x,-2))
}
##add a new column to the bed file called BIN_START
top_1PCT_WG_BED$BIN_START<-BIN_START(top_1PCT_WG_BED$POS)

top_1PCT_WG_BED$BIN_END<-BIN_END(top_1PCT_WG_BED$POS)
##
##remove the POS column to ensure the bed format is correct
top_1PCT_WG_BED<-select(top_1PCT_WG_BED,-POS)
head(top_1PCT_WG_BED)
####now write out the file in BED format
write.table(top_1PCT_WG_BED,"arenosa_lyrata_1PCT_divergent_SNPS.bed",row.names=F,
